...
public class ProducerAvroSchemaDBValidationsTypePAndroidHappyTest {
    protected static final Logger logger = LoggerFactory.getLogger(ProducerAvroSchemaDBValidationsTypePAndroidHappyTest.class);
    ...
    String primaryHId = "111111";
    String docClass = "";
    String GUID = '{' + generateUUID.toString() + '}';
    String applicationIdExpected = "";
    String messageType = "";
    DBConnections dbConn;
    String QUERY = "SELECT ident, type, format, source, received_date, status, payload FROM message WHERE type = \"push\" AND source = \"source\" AND  JSON_EXTRACT(payload, \"$.content.sharedData.appIdType\")=\"androidAppId\" AND  JSON_EXTRACT(payload, \"$.content.sharedData.originalEventRequestId\")=?";

    @Test
    public void produceMessageDBValidationsTypePushAndroidHappyTest() throws ExecutionException, InterruptedException, SQLException {
        ...
        ResultSet rs = dbConn.dbConn(ConfigUtils.getProperty(String.valueOf(ConfigUtils.ConfigKeys.ECORR_DB_URL_QA)), ConfigUtils.getProperty(String.valueOf(ConfigUtils.ConfigKeys.ECORR_DB_USERNAME_QA)), ConfigUtils.getProperty(String.valueOf(ConfigUtils.ConfigKeys.ECORR_DB_PASSWORD_QA)), QUERY, eventCorrelationId);
        ...
        if (rs.next()) {
            ...
            if (originalEventRequestID.equals(eventCorrelationId)) {

                assertThat(appIdType).isEqualTo("androidAppId");
                assertThat(triggeringEvent).isEqualTo("onsuccess");
                assertThat(triggeredTemplate).isEqualTo("dc-push-android");
                assertThat(originalMessageType).isEqualTo(messageType);
                ...
                assertThat(policyNumber).isEqualTo(primaryHId);
                ...
                assertThat(originalEventInitiator).isEqualTo(applicationIdExpected);
